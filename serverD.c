...
int main(){
    ...
    printf("The Server D has the following neughbor information:\n");
    ...
    printf("The Server D finishes sending its neighbor information to the Client with TCP port number %d and IP address ",ntohs(server.sin_port));
    for (int k=0; hp->h_addr_list[k]!= 0; k++){
        paddr((unsigned char*) hp->h_addr_list[k]);
    }
    printf("\n");
    printf("For this connection with the Client, the Server D has T​CP port number %d and IP address %s.\n", ntohs(myaddr.sin_port), inet_ntoa(myaddr.sin_addr));
